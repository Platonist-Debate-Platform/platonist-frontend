...
export const useHomepages = (update: boolean = false) => {
  ...
  const homepages = useSelector<GlobalState, GlobalState['homepages']>(state => state[PublicRequestKeys.Homepages]);
  ...
  useEffect(() => {
    if (!update && homepages.status === RequestStatus.Initial) {
      dispatch(requestAction.load(PublicRequestKeys.Homepages, {
        method: 'GET',
        url: url.href,
      }));
    }
    ...
    if (update && homepages.status === RequestStatus.Loaded) {
      dispatch(requestAction.update(PublicRequestKeys.Homepages, {
        method: 'GET',
        url: url.href,
      }));
    }
