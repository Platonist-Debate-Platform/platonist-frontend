...
export const useHomepage = (id: number, update: boolean = false) => {
  ...
  const homepage = useSelector<GlobalState, GlobalState['homepage']>(state => state[PublicRequestKeys.Homepage]);
  ...
  useEffect(() => {
    if (!update && homepage.status === RequestStatus.Initial) {
      dispatch(requestAction.load(PublicRequestKeys.Homepage, {
        method: 'GET',
        url: url.href,
      }));
    }
    ...
    if (update && homepage.status === RequestStatus.Loaded) {
      dispatch(requestAction.update(PublicRequestKeys.Homepage, {
        method: 'GET',
        url: url.href,
      }));
    }
