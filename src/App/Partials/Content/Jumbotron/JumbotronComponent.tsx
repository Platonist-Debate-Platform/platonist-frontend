    ...
    {textAlign === TextAlign.Full ? (
      <Col>
        <div className={`jumbotron-teaser jumbotron-teaser-${textAlign}`}>
          <div 
            className={`jumbotron-teaser-text text-${textAlign}`}
          >
            <Row>
              <Col md={10}>
                <h1>
      ...
      <Col 
        className={classNames({
          'offset-1 offset-md-2 offset-lg-3 offset-xl-4': textAlign === TextAlign.Center,
          'offset-2 offset-md-4 offset-lg-6 offset-xl-8': textAlign === TextAlign.Right,
        })}
        sm={10} 
        md={8}
        lg={6}
...
export const JumbotronComponent: React.FunctionComponent<JumbotronProps & {className?: string}> = (props) => {
  const {
    className,
    disableBackground,
    hasShape,
  ...
  const sm = config.breakpoints.find(breakpoint => breakpoint.type === BreakpointTypes.Sm);
    ...
    >
      ...
      {shape && (
        <div className={`jumbotron-shade jumbotron-shade-${textAlign}`}>
          <img className="img-fluid" src={shape} alt="" width={959} height={608} />
        </div>
      )}
    </Jumbotron>
  );
